...
func handleInquiry(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if nameLen == 0 {
			inruiryFormMessage.NameError = inruiryFormMessage.NameError + "お名前が入力されていません。"
		}
		if mailLen == 0 {
			inruiryFormMessage.MailError = inruiryFormMessage.MailError + "メールアドレスが入力されていません。"
		}
		if mailCheckLen == 0 {
			inruiryFormMessage.MailCheckError = inruiryFormMessage.MailCheckError + "メールアドレス確認が入力されていません。"
		}
		if questionLen == 0 {
			inruiryFormMessage.QuestionError = inruiryFormMessage.QuestionError + "お問い合わせ内容が入力されていません。"
		}

		// メールアドレス一致チェック
		if inruiryFormMessage.MailError == "" && r.Form.Get("mail") != r.Form.Get("mailCheck") {
